...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['funcion'] == 'insertar') {
    ...
    $imagen = limpiar_espacios($_POST['imagen']) ;
    $rubro = formato_capital(limpiar_espacios(strip_tags(trim($_POST['rubro'])))) ;
    $fecha = $_POST['fecha'];
    $id_editor = (int)$_POST['id_editor'];
    try {
        conectar();
        $sql = "INSERT INTO proyectos (nombre, detalle, imagen, rubro, fecha, id_editor)  
                VALUES ('$nombre', '$detalle', '$imagen', '$rubro', '$fecha', '$id_editor')";
        if (ejecutar($sql)) {
